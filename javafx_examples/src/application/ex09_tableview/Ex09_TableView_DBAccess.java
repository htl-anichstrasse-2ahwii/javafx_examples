...
import application.ex08_accessdatabase.DBManager;
 ...
 * 
 * Zusätzlich wird gezeigt, wie man die Daten in die Datenbank bekommmt und
 * wieder ausliest
 */
public class Ex09_TableView_DBAccess extends Application {
	...
	public void start(Stage primaryStage) {
		...
		Button bStore = new Button("speichern");
		bStore.setOnAction((ActionEvent e) -> {
			Person person = table.getSelectionModel().getSelectedItem(); // Hole den Index der ausgewählten Zeile
			String feedBackStr = String.format("Speichere Person %s %s", person.getFirstName(), person.getLastName());
		...
		top.getChildren().addAll(bAdd, bRemove, bChange,bStore, tFirstName, tLastName, tAge);
		root.setTop(top);
